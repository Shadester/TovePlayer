...
@synthesize totalPlaybackTime;


-(void)updateTimerFires {
    long currentPlaybackTime = self.musicPlayer.currentPlaybackTime;
    long remainingPlaybackTime = totalPlaybackTime - currentPlaybackTime;
    int currentHours = (remainingPlaybackTime / 3600);
    int currentMinutes = ((remainingPlaybackTime / 60) - currentHours*60);
    int currentSeconds = (remainingPlaybackTime % 60);
    timeLeftLabel.text = [NSString stringWithFormat:@"-%02d:%02d", currentMinutes, currentSeconds];
}
...
- (void)viewDidLoad {
    ...
    updateTimer = [NSTimer scheduledTimerWithTimeInterval:0.5
	                                         target:self
	                                       selector:@selector(updateTimerFires)
	                                       userInfo:nil
	                                        repeats:YES];
}
...
- (void)handleNowPlayingItemChanged:(id)notification {
    ...
    totalPlaybackTime = [[[musicPlayer nowPlayingItem] valueForProperty: @"playbackDuration"] longValue];
    int tHours = (totalPlaybackTime / 3600);
    int tMins = ((totalPlaybackTime/60) - tHours*60);
    int tSecs = (totalPlaybackTime % 60 );
    timeLeftLabel.text = [NSString stringWithFormat:@"-%02d:%02d", tMins, tSecs ];
